...
#@patch("module.adios2")
def test_kernels(config_analysis_cu):
    ...
    from analysis.kernels_spectral_cu import kernel_crossphase_cu, kernel_coherence_cu, kernel_crosspower_cu
    ...
    config_analysis_cu["storage"]["basedir"] = os.getcwd()
    ...
    nfft = config_analysis_cu["preprocess"]["stft"]["nfft"]
    ...
    my_preprocess = preprocessor(e, config_analysis_cu)
    my_tasklist = tasklist(e, config_analysis_cu)
    ...
    # fname_fq = os.path.join(os.getcwd(), "task_crossphase_cu" + f"_chunk{0:05d}_batch{0:02d}.npz")
