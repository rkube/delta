...
class loader_ecei():
    ...
    def gen_timebase(self, chunk_num):
        ...
        # The time base starts at t0. Assume samples are streaming in continuously
        # with sampling frequency fs. Offset of the first sample in current chunk
    ...
    def cache(self):
        ...
        with h5py.File(self.filename, "r",) as df:
            for ch in self.ch_range:
                ...
                self.cache[ch.idx(), :] = df[chname_h5][:self.chunk_size *
                                                        self.num_chunks].astype(np.float64)
    ...
    def batch_generator(self):
        ...
        for current_chunk in range(self.num_chunks):
            ...
            time_chunk = self.cache[:, current_chunk * self.chunk_size:
