...
def kernel_crossphase_64_cy(cnp.ndarray[cnp.complex128_t, ndim=3] data, ch_it, fft_config):
    ...
    cdef size_t ch2_idx
    cdef size_t idx, nn, bb # Loop variables
    cdef double complex _tmp, _tmp2
...
# This is version 1 where the cross-phase is calculated from the bin-averaged fourier modes
#            for nn in range(num_fft):
#                _tmp = 0.0
#                for bb in range(num_bins):
...
# This is version 2 where the cross-phase is calculated for each fourier-mode and then averaged over bins.
            for nn in range(num_fft):
                _tmp = 0.0
                for bb in range(num_bins):
                    _tmp2 = data[ch1_idx, nn, bb] * conj(data[ch2_idx, nn, bb])
                    _tmp += atan2(cimag(_tmp2), creal(_tmp2))
                _tmp = _tmp / num_bins
                result[idx, nn] = creal(_tmp)
