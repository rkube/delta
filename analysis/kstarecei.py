...
class KstarEcei(object):
    def __init__(self, shot, clist, ECEI_cfg):
        ...
        if shot < 19392:
            self.cnidx1 = 1
            self.dev = self.clist[0][0]
        else:
            self.cnidx1 = 2
            self.dev = self.clist[0][0:2]
        ...
        # data quality
        self.good_channels = np.ones(len(self.clist), dtype=np.bool)
        self.offlev = np.zeros(len(self.clist), dtype=np.bool)
        self.offstd = np.zeros(len(self.clist), dtype=np.bool)
        self.siglev = np.zeros(len(self.clist), dtype=np.bool)
        self.sigstd = np.zeros(len(self.clist), dtype=np.bool)
